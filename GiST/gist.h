...
public:
  ...
  rc_t create(const char *filename, gist_ext_t *ext);
  ...
  typedef rc_t (*TupleStream)(void *key, int &klen, void *data, int &dlen,
  ...
  rc_t create(const char *filename, gist_ext_t *ext, TupleStream s,
              void *sParam, float fillFactor);
  ...
  rc_t remove(const gist_query_t *query);
  ...
  rc_t fetch_init(gist_cursor_t &cursor, const gist_query_t *query, int k = 0,
                  int io = 0);

  // fetch the next data item;
  // gather profiling information for amdb (if profiling enabled)
  rc_t fetch(gist_cursor_t &cursor, void *key, smsize_t &keyLen, void *dataPtr,
             smsize_t &dataPtrLen, bool &eof);
  ...
  rc_t dump(std::ostream &s, shpid_t pgno = 0);
...
public:
  ...
  rc_t display(shpid_t pgno, JNIEnv *env, jint *highlights, jint numHighlights,
               jobject graphics, jobject normalColor, jobject highlightColor,
               jint width, jint height);

protected:
  void _getChildren(const gist_p &page,
                    Vector *children); // vector<shpid_t> *

  rc_t _getPredInfo(shpid_t subtreeRoot, // in
                    shpid_t parent,      // in
                    int levels,
                    DisplayPredInfo predInfo[], // out
                    int &next,                  // in
...
public:
  ...
  void setProfilingEnabled(amdb_wkldprofile *profile) { _profile = profile; }
  ...
  rc_t computeTreeMap(amdb_treemap *map);
  ...
  rc_t createOpt(const char *fileName, float fillFactor,
                 const amdb_wkldprofile &profile,
                 const amdb_clustering::Clustering &optClustering,
                 const amdb_treemap &map, Rid2ItemIdMap &ridMap);

  // write leaf level out as load file (ASCII)
  rc_t writeLoadfile(std::ostream &s);

  // locate target leaf, given insertion key
  rc_t locateTargetLeaf(const vec_t &key, // in: key to find leaf for
                        shpid_t &leaf);   // out: target leaf

private:
  // traverse the tree and compute the conversion maps
  // and profile->nodeStats recursively for single subtree
  rc_t _computeTreeMap(amdb_treemap *map, shpid_t pgno);
...
private:
  ...
  rc_t _locate_leaf(shpid_t root, gist_ustk &stack, const vec_t &key,
                    const vec_t &data);

  rc_t _fetch(gist_cursor_t &cursor, void *key, smsize_t &keyLen, void *dataPtr,
              smsize_t &dataPtrLen, shpid_t &leafNo, unsigned int &idx,
              bool &eof);

  rc_t _fetch_nn(gist_cursor_t &cursor, void *key, smsize_t &keyLen,
                 void *dataPtr, smsize_t &dataPtrLen, shpid_t &leafNo,
                 unsigned int &idx, bool &eof);

  rc_t _insert_leaf(gist_p &page, const vec_t &key, const vec_t &data,
                    vec_t &bp, bool &bpChanged);

  rc_t _apply_update(gist_p &page, int &leftIdx, const vec_t &leftBp,
                     const vec_t &rightBp, shpid_t rightChild, vec_t &bp,
                     bool &bpChanged);

  void _extract_bp(gist_ustk &stack, int stkIdx, vec_t &bp);

  rc_t _split_node(gist_p &left, gist_p &right, const int rightEntries[],
                   int numRight);

  rc_t _split(shpid_t root, gist_p &page, gist_ustk &stack, int stkIdx,
              const int rightEntries[], int numRight, gist_p &rightPage,
              vec_t &leftBp, vec_t &rightBp);

  rc_t _update_parent(shpid_t root, gist_ustk &stack, int stkIdx,
                      const vec_t &leftBp, const vec_t &rightBp,
                      shpid_t rightChild);

  rc_t _new_page(shpid_t root, gist_p &p, int2 level,
                 gist_file::page_descr *descr = NULL);

  rc_t _fix_page(gist_p &page, shpid_t pageNo, latch_mode_t mode)
  {
    return (_static_fix_page(_file, page, pageNo, mode));
  }

  void _unfix_page(gist_p &page) { _static_unfix_page(_file, page); }

  static rc_t _static_fix_page(gist_file &file, gist_p &page, shpid_t pageNo,
                               latch_mode_t mode);

  static void _static_unfix_page(gist_file &file, gist_p &page);
  ...
  void _check_node(gist_p &node, const vec_t &bpv);

  rc_t _check_sub(shpid_t pgno, const vec_t &pred, shpid_t parent, int slot,
                  ReachInfo reached[]);
  ...
  rc_t _build_level(TupleStream &tupStream, void *streamParam, float fillFactor,
                    int level, shpid_t rootPid, std::ostream &bpOutStream,
                    BreakNotifyFct breakNotify, int &pageCnt, gist_p &lastPage);

  rc_t _finalize_page(gist_p &page, std::ostream &outStream);

  rc_t _prepare_page(gist_p &page, shpid_t rootPid, int level);

  void _dump_pg(std::ostream &s, gist_p &page);

  rc_t _dump_sub(std::ostream &s, shpid_t pgno);

  static rc_t _readBpFromTemp(void *key, int &klen, void *data, int &dlen,
                              shpid_t &child, void *param);
  ...
  struct _OptTreeBuildState {
    ...
    ~_OptTreeBuildState() { clear(); }

    void clear(); // only called for internal levels, I think
    void update();
  };

  static rc_t _optItemStream(void *key, int &klen, void *data, int &dlen,
                             shpid_t &child, void *param);

  static rc_t _optBpStream(void *key, int &klen, void *data, int &dlen,
                           shpid_t &child, void *param);

  void _initLeafBuildState(const amdb_clustering::Clustering *optClustering,
                           const amdb_treemap *map, _OptTreeBuildState &state);

  rc_t _initInternalBuildState(_OptTreeBuildState &state);

  static void _optBreakNotify(int itemCnt, void *param);

  void _constructBpMap(ifstream &bpStream, amdb_treemap &bpMap);
