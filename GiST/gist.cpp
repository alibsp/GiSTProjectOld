...
rc_t gist::_new_page(shpid_t root,
    gist_p& p,
    int2 level,
...
rc_t gist::open(const char* filename)
{
...
rc_t gist::create(const char* filename, gist_ext_t* extension)
{
    _ext = extension;
...
rc_t gist::create(const char* filename,
    ...
    float fillFactor)
{
...
#else
    std::ofstream bpOutStream(temp1); // creates empty file
    std::ofstream tmpfile(temp2); // creates empty file
#endif
    // VCPORT_E
    ...
    if (fillFactor > 1.0)
        fillFactor = 1.0; // can't fill page more than once
...
rc_t gist::_locate_leaf(shpid_t root, // root of index
...
void gist::_extract_bp(gist_ustk& gstack, int stkIdx, vec_t& bp)
{
...
rc_t gist::_insert_leaf(gist_p& page, // leaf to insert on
...
rc_t gist::_apply_update(
    ...
    const vec_t&
        rightBp, // in: BP of new child (or NULL if orig. child not split)
    ...
    vec_t& rightBp) // BP for rightSib
{
    ...
    if (gstack.is_root(stkIdx)) {
        ...
        W_DO(_update_parent(
            root, gstack, stkIdx + 1, leftBp, rightBp, rightSib.pid()));
    }
...
rc_t gist::insert(const void* key, const int klen, const void* data, const int dlen)
{
    vec_t keyv(key, klen);
...
rc_t gist::_update_parent(shpid_t root,
    gist_ustk& gstack, // records ancestors
    ...
    shpid_t rightChild) // pointer to child's new sibling
{
    ...
    bool bpChanged;
    rc_t status = _apply_update(page,
        gstack.top(stkIdx)->idx,
        leftPred,
        rightPred,
        rightChild,
        bpv,
...
rc_t gist::remove(const gist_query_t* query)
{
#ifdef AMDB
    if (_breakHandler != NULL) {
        // construct bp info
...
rc_t gist::_static_fix_page(gist_file& file,
    gist_p& page,
    shpid_t pageNo,
    latch_mode_t mode)
{
...
void gist::_static_unfix_page(gist_file& file, gist_p& page)
{
...
rc_t gist::_fetch(gist_cursor_t& cursor,
    void* key,
    smsize_t& klen,
    void* data,
    smsize_t& dlen,
...
rc_t gist::_fetch_nn(gist_cursor_t& cursor,
    void* key,
    smsize_t& klen,
    void* data,
    smsize_t& dlen,
...
void gist::_check_node(gist_p& node, const vec_t& bpv)
{
...
rc_t gist::_check_sub(shpid_t pgno,
    const vec_t& pred,
    shpid_t parent,
    int slot,
    ReachInfo reached[])
    ...
    if (page.level() > 1) {
        ...
        for (int i = 0; i < recCnt; i++) {
            ...
            if (reached[childPtr].count > 0) {
                ...
                fprintf(stderr,
                    "additional reference to %d from %d, slot %d\n",
                    childPtr,
                    pgno,
                    i);
            } else {
...
rc_t gist::check()
{
    ...
    for (i = 0; i < numFree; i++) {
        ...
        info.isFree = true;
        if (info.count != 0) {
            cerr << "free node " << freeList[i] << " referenced " << info.count
                 << " times (orig. parent: " << info.parent << ")" << endl;
        }
    }
    delete[] freeList;
    ...
    for (i = 1; i < nodeCnt; i++) {
        ...
        if (info.count != 1 && !(info.count == 0 && info.isFree)) {
            // something's fishy
            cerr << "node " << i << " referenced " << info.count
                 << " times (orig. parent: " << info.parent << ")" << endl;
        }
    }
    delete[] reached;
...
void gist::_dump_pg(std::ostream& s, gist_p& page)
{
    int lvl = page.level();
    // dump header
    s << "level: " << lvl << " #slots: " << page.nrecs()
      << " avail: " << (int)page.usable_space() << endl;
...
rc_t gist::_dump_sub(std::ostream& s, shpid_t pgno)
{
    gist_p page;
...
rc_t gist::dump(std::ostream& s, shpid_t pgno)
{
    if (pgno != 0) {
        gist_p page;
        W_DO(_fix_page(page, pgno, LATCH_SH));
...
rc_t gist::_prepare_page(gist_p& page, shpid_t rootPid, int level)
{
...
rc_t gist::_finalize_page(gist_p& page,
    std::ostream& outStream) // receives BPs of finished pages
{
...
rc_t gist::_build_level(TupleStream& tupStream,
    void* streamParam,
    float fillFactor,
    int level,
    shpid_t rootPid,
    ...
    gist_p& page) // used as the current page to insert on
{
    ...
    for (;;) {
        ...
        if (breakPage || ((int)page.usable_space() - (int)gist_p::rec_size(klen, dlen) <= minFreeSpace)) {
    ...
    if (status != eEOF)
        return status;
    if (itemCnt == 0)
        return eFILEERROR; // can't have empty level
...
rc_t gist::_readBpFromTemp(void* key,
    int& klen,
    void* data,
    int& dlen,
    shpid_t& child,
    void* param)
{
    std::ifstream* s = (std::ifstream*)param;
