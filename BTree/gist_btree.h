...
public:
    ...
    // for printPred()
    typedef void (*PrintFct)(std::ostream& s, const void* pred);
    ...
    rc_t updateKey(gist_p& page, int& slot,const vec_t& newKey);
    void findMinPen( const gist_p& page, const vec_t& key, const vec_t& data, int& slot);
    void search( gist_p& page, const gist_query_t* query, int matches[], int& numMatches);
    ...
    bool check( const vec_t& bp, const vec_t& pred, int level);
...
extern bt_ext_t bt_str_key_binary_data_ext; // aldaghi- For Key tree
