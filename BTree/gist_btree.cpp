...
bt_ext_t::bt_ext_t(
    ...
    , keySize(keySize)
    , dataSize(dataSize)
    , negInftyKey(negInftyKey)
    , negInftyData(negInftyData) {};
...
void printInt( std::ostream& s, const vec_t& data)
{
    int i;
...
rc_t parseInt( istream& s, void* out, int& len)
{
    s >> *((int *) out);
...
rc_t parseInt(const char* str,  void* out, int& len)
{
    std::istrstream s(str, strlen(str));
    return parseInt(s, out, len);
}

rc_t _parseBtOp(std::istrstream& s, bt_query_t::bt_oper& oper)
{
...
rc_t parseIntQuery(const char* str, gist_query_t*& query)
{
    std::istrstream s(str, strlen(str));
    rc_t status;
